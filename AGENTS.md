...
5. **Export** — a completed roast can be downloaded as CSV
   (`/roasts/[id]/export`). Built — see "Export" below. (A GitHub Pages
   publish feature existed here too; removed 2026-09 — see "Publish feature
   removed" below for why.)
...
**Export:** `/roasts/[id]/export` (a route handler,
`src/app/roasts/[id]/export/route.ts`) streams a CSV — a metadata block
(bean, weights, level, rating, duration) followed by the full event table
(`src/lib/csv.ts`). (This repo also had a GitHub Pages publish feature —
`publishRoast`/`unpublishRoast`, a `RoastSession.publishedAt` column,
static HTML rendering — removed 2026-09; see "Publish feature removed"
below.) The roasting-curve SVG is generated by `buildRoastCurveSvg`
(`src/lib/curve.ts`) — a single pure function the live React chart
(`RoastCurveChart.tsx`) renders via `dangerouslySetInnerHTML`. `curve.ts`
also exports `getCurveReadings`/`getChartLayout` (the same pixel math
`buildRoastCurveSvg` draws from) so `RoastCurveChart.tsx` — a client
component — can compute an interactive overlay (hover crosshair + tooltip
showing time/temp/fan/heat, snapped to the nearest real reading, never
interpolated) without duplicating that math and risking it drifting from
...
`--ror` (`globals.css`) is the one deliberately cool tone in an otherwise
warm palette — distinct from
...
**Publish feature removed (2026-09).** Everything from here through "Bulk
historical import" below describes a GitHub Pages publish feature
(`publishRoast`/`unpublishRoast`, `src/lib/publish.ts`, `publish-url.ts`,
`src/lib/git.ts`, `RoastSession.publishedAt`, the `docs/` output directory)
that has since been **removed** — it wasn't earning its maintenance cost
for a personal-use app, and its output was stale HTML nothing else
depended on. Kept below as historical record: the incidents and gotchas
are real lessons that would recur if a similar feature were ever rebuilt.
Don't treat any of it as describing current app behavior.

**Publish/unpublish used to commit and push `docs/` automatically**
...
read: anything scoped to completed roasts (CSV, `computeHistoricalBaseline`,
...
`<option>` elements and can't hold rich markup, so they stay text.
Stock/progress bars (`BeanStockBar`,
